...
2.  **動態日期範圍注入 (v1.4.0+)**：
    *   **偵測機制**：分析選定的 JSON Query，若包含 `@timestamp` 的 `range` 運算子，主動提示使用者自訂。
    *   **預設處理**：若使用者不自訂 (選擇 No/Esc)，系統會自動生成「系統時間起算 24 小時」的範圍填入。
    *   **替換邏輯**：使用遞迴搜尋 JSON 結構，精確替換所有 `@timestamp` 的 `gte` (大於等於) 與 `lte` (小於等於) 值。
3.  **執行匯出 (核心邏輯)**：
...
*   **自動化導出 (含日期注入)**：
    ```bash
    ./osdx export --profile Prod-Logs --query Errors --from 2026-03-01 --to 2026-03-05
    ```
...
3.  **日期範圍確認**：若查詢包含時間過濾條件，則進入日期詢問環節。
4.  **導出設定與執行**：
    *   **格式選擇**：使用上下鍵選擇 CSV 或 JSON。
    *   **確認執行**：顯示設定摘要，詢問「是否開始導出？[Y/n]」。
    *   **視覺化進度**：執行期間顯示彩色進度條，實時顯示「已匯出筆數」與「已執行時間」。
...
### 5.6 動態日期範圍注入 (Dynamic Date Range Injection)
為了讓使用者在導出前能靈活調整時間，而無需反覆修改靜態查詢語句，系統導入了智慧型日期注入機制：
1.  **JSON 結構掃描**：系統遍歷 `JsonElement` 節點。
2.  **偵測目標**：鎖定帶有 `"range"` 運算子且鍵名為 `"@timestamp"` 的節點。
3.  **智慧型替換**：
    *   若輸入不含時間部分 (如 `2026-02-20`)，自動補足起始 (`00:00:00`) 與結束 (`23:59:59`) 時分秒。
    *   支援精確 ISO 8601 格式，確保 OpenSearch 解析無誤。
    *   **預設範圍生成**：當使用者按 Esc 或 No 時，系統使用 `DateTime.Now` 與 `DateTime.Now.AddHours(-24)` 自動動態生成具體的 UTC/Local 時間字串，確保存取最新資料。
