...
4.  **日期範圍自訂 (v1.4.0+)**：
    *   系統會自動偵測查詢語句中是否包含 `@timestamp` 的範圍條件。
    *   若偵測到，系統會詢問：`是否要自訂日期範圍？`
    *   **選擇 No 或按 Esc**：直接使用系統時間起算「最近 24 小時」作為預設範圍。
    *   **選擇 Yes**：手動輸入起始與結束時間。
        *   支援格式：`2026-02-20` (日期) 或 `2026-02-20T10:30:00` (精確時間)。
        *   驗證機制：若格式錯誤或結束時間早於起始時間，系統會要求重新輸入。
5.  程式將顯示進度條，實時呈現：
    *   **已匯出筆數 / 總預估筆數**
    *   **進度百分比**
    *   **預估剩餘時間**
...
OSDX 支援透過命令列參數直接執行導出任務，適合整合至 Cron 或工作排程器。

### 基本指令
```bash
./osdx export --profile [Profile名稱] --query [Query名稱] [其他參數]
```

### 支援參數
*   **`--profile`, `-p`** (必填)：指定已存在的 Profile 名稱。
*   **`--query`, `-q`**：指定該 Profile 下的查詢語句名稱（預設為 `Default`）。
*   **`--username`, `-u`**：覆蓋連線帳號。
*   **`--password`, `-pass`**：連線密碼（建議透過環境變數 `OSDX_PASSWORD` 傳遞）。
*   **`--output`, `-o`**：覆蓋輸出路徑。
*   **`--batch-size`, `-b`**：覆蓋批次抓取數量。
*   **`--from` (v1.5.1+)**：指定起始日期 (gte)，格式為 `yyyy-MM-dd` 或 ISO 8601。
*   **`--to` (v1.5.1+)**：指定結束日期 (lte)，格式為 `yyyy-MM-dd` 或 ISO 8601。

### 日期參數行為
*   若同時指定 `--from` 與 `--to`，系統將完整替換查詢中的 `@timestamp` 範圍。
*   若僅指定其中之一，另一端將自動補足（預設範圍為 24 小時）。
*   格式範例：`--from 2026-03-01` 或 `--from 2026-03-01T10:00:00`。
